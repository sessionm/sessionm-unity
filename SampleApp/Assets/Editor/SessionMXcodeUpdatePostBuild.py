...
def main(argv = sys.argv, pluginName = 'SessionM'):
    unityRootProject = os.getcwd()
    sys.stdout = Logger(unityRootProject + '/SessionMXcodeUpdatePostBuild.log')
    ...
    print '--- Updating Unity-iPhone.xcodeproj/project.pbxproj for Unity integration ---'
    try:
        projectFile = projectPath + '/Unity-iPhone.xcodeproj/project.pbxproj'
        pluginPath = unityRootProject + '/Assets/Plugins/iOS/'
        p = XcodeProject(projectFile)
        p.projectPath = projectPath
        weakFrameworks = ['CoreTelephony.framework', 'AdSupport.framework']
        requiredFrameworks = weakFrameworks + ['AdSupport.framework',
         ...
         'ImageIO.framework']
        for f in requiredFrameworks:
            path = 'System/Library/Frameworks/' + f
            ext = os.path.splitext(f)[1].lower()
            fileHash = p.addFileReference(f, path, 'SDKROOT', 'Frameworks', False)
            if fileHash is not None:
                buildFileHash = p.addBuildFile(f, fileHash, f in weakFrameworks)
                p.addFileToBuildPhase(f, buildFileHash, 'Frameworks')
                continue
        ...
        return 1
    print '--- Finished SessionM integration ---'
...
class Logger(object):
    ...
    def __init__(self, filename = 'SessionMXcodeUpdatePostBuild.log'):
